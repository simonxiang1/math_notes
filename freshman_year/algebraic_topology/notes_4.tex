...
Covering spaces of $S^{1} \vee S^{1} $ form a rich family that demonstrate the general theory very concretely. For convenience, let $X=S^{1} \vee S^{1} $. View it as a graph with one vertex and two edges, with the edges labeled $a$ and $b$. Let $\widetilde X$ be any other graph with four edges connected to each point, like $X$ at its singular vertex, and that each edge has been assigned an orientation like the ones assigned to each edge of $X$. That is, for each vertex there are two $a$-edges and $b$-edges oriented toward and away from the vertex. Help I can't include figures that are the proper size! Let's call $\widetilde X$ a $2$-oriented graph.
...
This covering space is called the \emph{universal cover} of $X$, because it covers every connected covering spcae of $X$.
